...
Let us now download the results for python simulation, created using `./python/tdavec/time_tests.py` script

```{r}
py_lines <- readLines("../python/tdavec/py_times.txt")
py_cost <- rbind(
  t(cost),
  t(sapply(3:length(py_lines), function(i) 1000*as.numeric(strsplit(py_lines[i], ",")[[1]][4])))
)
